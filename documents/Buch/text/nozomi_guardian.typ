...
Die Nozomi Guardian nennt alle Geräte in einem Netzwerk Assets. Über diese Assets bringt sie so viel wie Möglich in Erfahrung. Sei dies von einer IP-Adresse bis hin zu dem laufenden Betriebssystem oder Firmware-Version. Zu jedem Asset wird auch ein risk assessment erstellt, in welchem auf bekannte und erkannte Vulnerabilitäten eingeganen wird. 
...
=== Angriffe von innen
Ein weiterer Angriffsvektor sind Angriffe von innen. Dies kann durch unzufriendene Mitarbeiter oder auch durch einen versehentlichen Klick auf eine Phishing-Mail bzw. das Anstecken eines USB-Sticks passieren. Doch auch hier kann die Guardian helfen. Sie erkennt, wenn ein Gerät plötzlich anfängt, ungewöhnliche Datenpakete zu senden und kann so einen Angriff erkennen.
...
=== Maleware via Wechseldatenträger
Ein unschuldig aussehender USB-Stick kann schnell zu einem Angriffsvektor werden. Er wird angesteckt um einfach nur zu kontrollieren, wem der USB-Stick gehört und sobald dies geschieht ist der Angriff schon im Gange. Der optimalste Weg ist hier zwar die Schulung von Mitarbeitern und Mitarbeiterinnen, doch selbst dies kann nicht 100% aller Angriffe verhindern. Die Nozomi Guardian füllt das fehlende Loch, indem sie die Angriffe erkennt und Alarm schlägt sobald er auffällt.

=== Maleware via Inter- oder Intranet
